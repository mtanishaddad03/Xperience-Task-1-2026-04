...
| **D5** | **An email address is invited at most once per event**, compared after normalisation. A repeated address is skipped individually, never failing the batch. **An invalid address does fail the batch:** the whole batch is rejected and every invalid address is listed, so the host fixes the list and resubmits *(before Stage 2)*. | Prevents one person holding two places — and makes the host's largest operation, a batch of hundreds, safe to retry. | — |
...
| **D14** | **Per-host invitation limit (Q8): at most 1,000 invitations per verified host address in any rolling 24 hours, across all of that host's events.** A batch that would exceed it is **rejected whole**, with a clear message. **What counts:** every invitation message created for that address's events, **including resends** (U10) — each is an email sent; addresses skipped as already invited (D5) do not count, and neither do automatic retries of the same message (D15). **Management-link messages are capped too: at most 5 per recipient address per rolling 24 hours** — U1 has no credential, so without this anyone could make the system email any address with their own title and description. Both limits are counted and checked under a transaction-scoped advisory lock keyed on the normalised address, so two simultaneous requests cannot both slip under a limit. **Exceeding the management-link cap refuses U1 and creates nothing** — an event whose link was never sent could never be managed, which is worse than revealing that an address was used five times today. *(Before Stage 2.)* | Covers the normal 600-guest event (F15) with room to spare, while limiting a host who opens many events to multiply their allowance (W9). Rejecting whole keeps the host's list either fully invited or not at all — never an arbitrary prefix of it. | A host with more than 1,000 guests must spread invitations over two days. A determined abuser with fresh addresses is still only slowed (W9). |
...
| **U2** Invite guests | Host submits addresses | Resolve management link; require verified host; **require the event Open and not started**; reject the whole batch if any address is invalid (listing them) or if it would exceed D14; create each guest with an invitation message, skipping existing addresses | Guests created; messages queued | D5, D8, D9, Q8 |
